...
void opening() {
  ...
  while (1) {
    ...
    for (int i = 0; i < kuina_num; i++) {
      if (kuinas[i].x > 0) {
        ...
        ssd1306_drawImage(kuinas[i].x, kuinas[i].y, kuinas[i].img, kuinas[i].w,
                          kuinas[i].h, 0);
      }
      kuinas[i].x += 1;
    }
  ...
  character mongoose = {
      2, mongoose_left_1, mongoose_left_1, mongoose_left_2, 1, 0, 30, 16, 8};
  ...
  while (1) {
    ...
    ssd1306_drawImage(mongoose.x, mongoose.y, mongoose.img, mongoose.w,
                      mongoose.h, 0);
...
bool game_loop() {
  ...
  while (loop_flag) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      // draw enemy
      if (enemy[i].show) {
        ssd1306_drawImage(enemy[i].x, enemy[i].y, enemy[i].img, enemy[i].w,
                          enemy[i].h, 0);
